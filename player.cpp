...
void Player::updatePosition(SelectablePolygonF **colliders, int n_colliders)
{
    //---------------------//
    // Collision detection //
    //---------------------//
    ...
    for(int i = 0; i < COLLSION_ITER && (_xBlocked || _yTopBlocked || _yBottomBlocked); i++){
        ...
        // Checking every colliders if their bounding boxes are in close proximity to players next move.
        // If that's the case, add them to a vector and discard the rest.
        QVector<SelectablePolygonF *> boundObjects;

        QRectF playerBounds(QPointF(qMin(_pos.x(), _pos.x() + nextMove.x()),
        ...
        for(int c = 0; c < n_colliders; c++){
            QRectF bounds(QPointF(qMin(playerBounds.left(), colliders[c]->boundingRect().left()),
                          qMin(playerBounds.top(), colliders[c]->boundingRect().top())),
                          QPointF(qMax(playerBounds.right(), colliders[c]->boundingRect().right()),
                          qMax(playerBounds.bottom(), colliders[c]->boundingRect().bottom())));

            if(bounds.width() < colliders[c]->boundingRect().width() + playerBounds.width() &&
               bounds.height() < colliders[c]->boundingRect().height() + playerBounds.height()){
    ...
    //Decelerate the player's x movement if left or right wasn't pressed
    if(!moveRequest){
        if (_speed.x() < 0)
            _speed.rx() += _dec.x();
        if (_speed.x() > 0)
...
void Player::checkItems(Item **items, int n_items){
    QVector<Item *> boundItems;

    QRectF playerBounds(QPointF(qMin(_pos.x(), _pos.x() + _speed.x()),
    ...
    for(int c = 0; c < n_items; c++){
        QRectF bounds(QPointF(qMin(playerBounds.left(), items[c]->getHitbox().left()),
                      qMin(playerBounds.top(), items[c]->getHitbox().top())),
                      QPointF(qMax(playerBounds.right(), items[c]->getHitbox().right()),
                      qMax(playerBounds.bottom(), items[c]->getHitbox().bottom())));

        if(bounds.width() < items[c]->getHitbox().width() + playerBounds.width() &&
           bounds.height() < items[c]->getHitbox().height() + playerBounds.height()){
...
void Player::keyInput(int key, bool down, QKeyEvent *e)
{
    switch(key) {
    case Qt::Key_Space:
...
QPixmap *Player::getCurrentPixmap()
{
...
QPointF Player::linearMovement(QPointF pixelsPerSecond){
